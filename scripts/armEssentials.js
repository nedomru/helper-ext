...
.ui-accordion-header > form:nth-child(3) > div:nth-child(4) {
  ...
  const observer = new MutationObserver(() => {
    const top3 = document.querySelector("#top-3-block");
    if (top3) {
      console.log("нашел formform");
      document.head.appendChild(style);
      observer.disconnect();
    } else console.log("не нашел форму");
  });

  console.log("ищем formform");
...
if (
  ...
  document.URL.indexOf(
    "ertelecom.ru/cgi-bin/ppo/excells/wcc2_main.frame_main"
  ) != -1
) {
  console.log("нашлись основной фрейм");
  armMain();
}
