...
async function applyTheme() {
    let darkTheme = await browser.storage.sync.get("OTHER_DarkTheme_ARM").OTHER_DarkTheme_ARM
    ...
    if (
        document.URL.includes("ertelecom.ru/static_pages/private/cashback_movix")
    ) {
        await movixCashback();
    }
    ...
    if (
        document.URL.includes(
            "ertelecom.ru/static_pages/private/wcc/transfer_money_fz"
        )
    ) {
        await transferMoney();
    }
    ...
    if (
        document.URL.includes(
            "ertelecom.ru/static_pages/private/reversal_additional_charges"
        )
    ) {
        await storno();
    }
