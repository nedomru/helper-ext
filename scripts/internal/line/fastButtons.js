...
async function dutyButtons() {
  ...
  const linksData = [
    ...
    },
    {
      ...
      url: "https://okc2.ertelecom.ru/yii/genesys-reports/ntp/sl-forecast",
...
async function specialistButtons() {
  if (
    document.querySelector(".helper-specialist-button") !== null ||
  ...
  const interval = setInterval(() => {
    ...
    if (lineHeader !== null) {
      lineHeader.parentNode.insertBefore(
        buttonToggleContainer,
        lineHeader.nextSibling,
      );
  ...
  const buttonData = [
    ...
    {
      text: "ОКЦ",
      link: "https://okc2.ertelecom.ru/yii/#octpNck",
  ...
  buttonData.forEach((item) => {
    if (item.show) {
      ...
      button.className =
        "v-btn v-btn--contained v-size--default helper-specialist-button";
