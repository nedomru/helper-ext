...
async function initInformationFold() {
    new MutationObserver((mutations) => {
        ...
        try {
            const rowsToHide = ["Статус:", "Схема оплаты:", "", "Категория:", "Контактные e-mail:", "Место рождения", "Активные приложения", "Промо-пакет", "Идентификационные данные", "Автоплатеж",];
            ...
            if (totalHidden > 0) {
                ...
                toggleButton.addEventListener("click", (e) => {
                    ...
                    document
                        .querySelectorAll('[helper-hidden-row="true"]')
                        .forEach((row) => (row.style.display = isHidden ? "table-row" : "none"),);

                    // Update button and status text
                    toggleButton.textContent = isHidden ? "🔽 Свернуть строки" : `▶️ Развернуть строки`;
                    toggleButton.setAttribute("data-state", newState);

                    status.textContent = isHidden ? "Отображены все поля" : "Дополнительные поля скрыты";
...
/**
 * Folds inactive and closed applications from applications tab
 */
        ...
        try {
            ...
            if (totalHidden > 0 && !appsTab.querySelector("#helper-toggle-apps")) {
                ...
                toggleButton.addEventListener("click", (e) => {
                    ...
                    // Update button and status text
                    toggleButton.textContent = isHidden ? "🔽 Свернуть приложения" : `▶️ Развернуть приложения (${totalHidden})`;
                    toggleButton.setAttribute("data-state", newState);

                    status.textContent = isHidden ? "Отображены все приложения" : "Неактивные и закрытые приложения скрыты";
                    status.style.color = isHidden ? "#198754" : "#dc3545";
                });
...
/**
 * Folds intermediate steps of appeals
        ...
        try {
            ...
            appeals.forEach((appeal, index) => {
                ...
                toggleButton.addEventListener("click", (e) => {
                    ...
                    document
                        .querySelectorAll(`[appeal-number="${appealNum}"][appeal-step="intermediate"]`,)
                        .forEach((row) => (row.style.display = isHidden ? "table-row" : "none"),);

                    toggleButton.textContent = isHidden ? `🔽 Шаги отображены` : `▶️ Шаги скрыты (${hiddenSteps})`;
                    toggleButton.setAttribute("data-state", newState);
            ...
            if (totalHidden > 0) {
                ...
                toggleBtn.addEventListener("click", (e) => {
                    ...
                    toggleButtons.forEach((btn) => {
                        const appealNum = btn
                            .closest("tr")
                            .previousSibling.getAttribute("appeal-number");
                        const hiddenCount = document.querySelectorAll(`[appeal-number="${appealNum}"][appeal-step="intermediate"]`,).length;
                        btn.textContent = isHidden ? "🔽 Шаги отображены" : `▶️ Шаги скрыты (${hiddenCount})`;
                        btn.setAttribute("data-state", newState);
                        btn.style.color = isHidden ? "#006400" : "#ff0000";
                    });

                    document
                        .querySelectorAll('[appeal-step="intermediate"]')
                        .forEach((row) => (row.style.display = isHidden ? "table-row" : "none"),);

                    toggleBtn.textContent = isHidden ? "🔽 Шаги отображены" : `▶️ Шаги скрыты`;
                    toggleBtn.setAttribute("data-state", newState);

                    status.textContent = isHidden ? "Отображены все шаги" : "Промежуточные шаги скрыты";
                    status.style.color = isHidden ? "#198754" : "#dc3545";
                });
...
/**
 * Hides intermediate steps from service requests
 */
async function initServiceRequestsFold() {
    const COMPLETION_STATUSES = ["Выполнено (Отправить в Call-центр)", "Заявка выполнена", "Выполнено", "Помощь оказана",];
                ...
                if (COMPLETION_STATUSES.includes(lastRowStatus)) {
                    ...
                    if (middleRows.length > 0) {
                        ...
                        toggleButton.addEventListener("click", (e) => {
                            ...
                            toggleButton.textContent = isHidden ? `🔽 Шаги отображены` : `▶️ Шаги скрыты (${middleRows.length})`;
                            toggleButton.setAttribute("data-state", newState);
                            toggleButton.style.color = isHidden ? "#006400" : "#ff0000";
                        });
...
/**
 * Folds intermediate steps of connection requests
 */
...
/**
 * Removes unnecessary tabs from EQM page
 */
...
/**
 * Removes unnecessary columns from Appeals page
 */
